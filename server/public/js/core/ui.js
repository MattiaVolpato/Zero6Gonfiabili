// --- UI HELPERS: Gestione Interfaccia Utente ---
/**
 * Raccolta di funzioni di utilità per manipolare il DOM e gestire componenti UI comuni:
 * 
 * 1. z6UpdateActiveNav: Gestisce l'evidenziazione del link attivo nella navbar in base all'URL corrente.
 * 2. Gestione Flash Messages (setFlash, renderFlashIfAny): Sistema per mostrare notifiche temporanee (success/error)
 *    che sopravvivono al refresh o alla navigazione (salvati in sessionStorage).
 * 3. Utilities varie:
 *    - debounce: Ottimizza le prestazioni ritardando l'esecuzione di funzioni frequenti (es. input, resize).
 *    - formatDisplayName: Formatta il nome utente per la visualizzazione.
 *    - renderStarsCompact: Genera le stelline per le recensioni.
 *    - escapeHtml: Sanitizza le stringhe per prevenire XSS (Cross Site Scripting, che permette di eseguire codice dannoso).
 */
