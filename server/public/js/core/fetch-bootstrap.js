// --- FETCH WRAPPER: Gestione Automatica Richieste ---
/**
 * Questo modulo sovrascrive globalmente `window.fetch` per semplificare le chiamate API:
 * 
 * 1. CSRF Automatico: Aggiunge l'header "X-CSRF-Token" a tutte le richieste che modificano dati (POST, PUT, DELETE, ecc.).
 * 2. Credenziali: Imposta di default `credentials: "include"` per inviare sempre i cookie di sessione.
 * 3. Gestione Errori 403: Se il token CSRF è scaduto (errore 403), tenta automaticamente 
 *    di rinnovare il token e riprovare la richiesta in modo trasparente senza disturbare l'utente.
 */
