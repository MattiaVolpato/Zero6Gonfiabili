...
// me
r.get("/me", (req, res) => {
  /*Grazie a Passport.js, se un utente è loggato, i suoi dati si trovano nell'oggetto req.user.
   Se req.user non esiste, significa che l'utente è un ospite (guest) e il server risponde con null. 
   Questo è fondamentale per il frontend per capire se mostrare il form di login o il profilo utente. */
  ...
  passport.authenticate("local", async (err, user, info) => {
    ...
    req.session.regenerate((regenErr) => {
      ...
      req.login(user, (loginErr) => {
        ...
        /*Trasferimento Dati Utente: Invia i dati essenziali dell'utente (id, email, nome, ruolo) 
        in modo che il client possa aggiornare l'interfaccia, ad esempio mostrando il nome dell'utente
        nell'header o abilitando il tasto per il pannello admin se role: "admin" */
        return res.json({
          user: {
            id: user.id,
            email: user.email,
